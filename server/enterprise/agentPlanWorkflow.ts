...
import { PLAN_LIMITS, PlanError, parsePlan, parseSubagentResult, planSchema, subagentResultSchema, type PlanScope, type PlanWorkspace, type PlanVersion } from "../../src/enterprise/agentPlan";
...
export class AgentPlanWorkflow {
	...
	command(scope: PlanScope, actorId: string, body: unknown): PlanWorkspace {
		...
		if (typeof p.requestId !== "string" || !/^[A-Za-z0-9._:-]{1,128}$/.test(p.requestId) || !Number.isInteger(p.revision) || !["mode", "generate", "confirm", "pause", "resume", "cancel"].includes(String(p.action))) throw new PlanError("invalid_plan_command", 400);
		...
		const result = this.store.change(scope, `user:${p.requestId}`, hash(p), actorId, `plan.${p.action}`, Number(p.revision), (state) => {
			...
			if (p.action === "mode") {
				...
				if (typeof p.objective !== "string" || !p.objective.trim() || p.objective.length > 8000) throw new PlanError("invalid_plan_objective", 400);
				if (current && ["awaiting_confirmation", "paused", "failed"].includes(current.status)) current.status = "superseded";
				state.versions.push({ version: state.versions.length + 1, objective: p.objective.trim(), context: input.context, conversationRevision: input.revision, status: "planning", createdAt: this.now(), actorId, generation: 1, calls: 0, children: [] });
			} else {
	...
	dispatch(scope: PlanScope) {
		...
		if (existing?.status === "dead_letter" || existing?.status === "cancelled") {
			this.store.change(scope, `job-terminal:${jobId}`, jobId, "host", "plan.failed", state.revision, (s) => { latest(s)!.status = "failed"; latest(s)!.failure = { code: "plan_job_failed", retryable: true }; });
			return;
		}
		if (existing) return;
	...
	async execute(lease: StageJobLease, signal: AbortSignal, assertActive: () => void): Promise<StageJobHandlerResult> {
		...
		try {
			...
			if (!response) {
				...
				response = await this.runtime.executeTurn({
					...
					instructions: [...this.options.instructions,
						...
						planning ? `Plan only. Do not execute the task. Propose 1-${PLAN_LIMITS.tasks} independent, bounded tasks with their objective and tools. Each task gets a separate context and cannot read sibling outputs. Allowed execution tools: ${this.options.executionTools.join(", ")}. No recursive delegation or background work. Match the user's language.` : "Execute ONLY this approved subtask. You are an isolated subagent; no delegation or background tasks. File writes/deletes require their own Host approval. Report actual evidence and limitations. Never claim approval, verified business facts or production readiness. Match the user's language.",
					],
					input: JSON.stringify({ objective: planning ? plan.objective : task!.objective, sourceContext: plan.context }),
			...
			const result = planning ? undefined : parseSubagentResult(parsed);
			state = this.store.read(scope);
			this.store.change(scope, `result:${key}`, hash(parsed), plan.actorId, planning ? "plan.proposed" : "subagent.completed", state.revision, (s) => {
				const p = latest(s)!;
				if (spec) { p.spec = spec; p.status = "awaiting_confirmation"; }
					...
					delete p.activeTask;
					p.status = p.children.length === p.spec!.tasks.length ? "completed" : "running";
				}
			});
			return planning || index + 1 === plan.spec!.tasks.length ? { status: "completed" } : { status: "paused", sessionId };
		} catch (error) {
			...
			if (latest(current)?.version === version && latest(current)?.generation === generation && active(latest(current))) {
				...
				this.store.change(scope, `failure:${randomUUID()}`, code, "host", "plan.failed", current.revision, (s) => { latest(s)!.status = "failed"; latest(s)!.failure = { code, retryable: error instanceof RuntimeFailure && error.retryable }; });
			}
