...
export function ModelMonitor({ conversationId, language }: { conversationId: string; language: Language }) {
	...
	return <section className="model-monitor" aria-label={en ? "Model call monitor" : "模型调用监控"}>
		...
		{error && <p role="alert" className="file-error">{errorText(error, language)} · {en ? "the data below may be stale" : "下方数据可能已过期"}</p>}
		{!view ? <p>{en ? "Loading call records…" : "正在加载调用记录…"}</p> : <>
			...
			<dl className="run-metadata"><div><dt>{en ? "Waiting / cancelled or interrupted" : "等待 / 取消或中断"}</dt><dd>{summary!.running} / {summary!.cancelled}</dd></div><div><dt>{en ? "Token count requests" : "Token 计数请求"}</dt><dd>{summary!.counted}</dd></div><div><dt>{en ? "Uncached input / output tokens" : "非缓存输入 / 输出 Token"}</dt><dd>{summary!.inputTokens} / {summary!.outputTokens}</dd></div><div><dt>{en ? "Cache read / write tokens" : "缓存读取 / 写入 Token"}</dt><dd>{summary!.cacheCoverage ? `${summary!.cacheReadTokens} / ${summary!.cacheWriteTokens}` : en ? "Not provided" : "未提供"}</dd></div></dl>
			<p className="panel-note">{en ? `Cache fields cover ${summary!.cacheCoverage} / ${summary!.responses} responses. Responses without cache fields are excluded. Rate = cache reads ÷ (uncached input + cache reads + cache writes).` : `缓存统计覆盖 ${summary!.cacheCoverage} / ${summary!.responses} 次有效响应；上游缺少缓存字段时不计入命中率。按缓存读取 ÷（非缓存输入 + 缓存读取 + 缓存写入）计算。`}</p>
			<p className="panel-note">{en ? `The latest ${view.retentionLimit} requests per run survive refresh and restart. ${view.truncated ? "Earlier records have expired; statistics cover the retained window." : "Only requests made since monitoring was enabled are included."}` : `每个运行最多保留最近 ${view.retentionLimit} 次请求，刷新与重启后保留。${view.truncated ? "更早记录已移出，以上为保留窗口统计。" : "仅统计此功能启用后的请求。"}`}</p>
